<template>
  <el-card class="form-container" shadow="never">
    <el-form :model="productAttr" :rules="rules" ref="productAttrFrom" label-position="left" label-width="150px">
      ...
      <el-form-item label="属性值可选值列表:">
        <el-input :autosize="true" type="textarea" v-model="productAttr.list"></el-input>
      </el-form-item>
  ...
  export default {
    ...
    created() {
      if(this.isEdit){
        getProductAttr(this.$route.query.id).then(response => {
          const num=this.$route.query.rid;
          for(let i=0;i<response.data.list.length;i++){
              if(this.$route.query.rid==response.data.list[i].id){
                  this.productAttr = response.data.list[i];
                  break;
                  //this.inputListFormat = this.productAttr.list.replace(/,/g,'\n');
              }
            }
          });
      }else{
    ...
    watch:{
      inputListFormat: function (newValue, oldValue) {
        ...
        this.productAttr.list = newValue;
      }
    },
    methods: {
      // getCateList() {
      ...
      resetProductAttr() {
        ...
        this.productAttr.cId = Number(this.$route.query.cid);
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$confirm('是否提交数据', '提示', {
              ...
              if(this.isEdit){
                //this.productAttr.id=this.$route.query.rid
                ...
                //this.productAttr.id=Number(this.$route.query.cid)
                //this.productAttr.cid=this.$route.query.cid
